function [integration_error, Nt, ft_5, integral_1000] = zadanie2()
    ...
    for i=1:length(Nt)
        N = Nt(i);
        integration_result = integral_trapezoid(@f, N, 0, 5);
        integration_error(i) = abs(integration_result-reference_value);
    end

    ft_5 = f(5);
    integral_1000 = integral_trapezoid(@f, 1000, 0, 5);
...
function r = integral_trapezoid(func, N, a, b)
    r = 0;
    x = linspace(a, b, N+1);
    for i=1:N
        r = r + (func(x(i))+func(x(i+1)))/2;
    end
